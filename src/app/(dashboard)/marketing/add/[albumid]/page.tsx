  ...
  useEffect(() => {
    const fetchData = async (labelId: string, albumId: string) => {
      try {
        ...
        const response = await apiGet(`/api/marketing/get?albumId=${albumId}&labelId=${labelId}`);
        console.log("Data from fetch response API:");
        console.log(response);
        console.log("isExtraFileRequested:", response.data.isExtraFileRequested);
  ...
  return (
    <div className="w-full min-h-screen p-6 bg-white rounded-sm">
      ...
      <h1 className="text-3xl font-bold mb-4 mt-5 text-blue-600">
        {albumId ? `Update Marketing for ${albumName}` : `Add New Marketing for ${albumName}`}
      </h1>
